  ...
  const fetchApplications = async () => {
    try {
      ...
      if (activeTab === 'pending') {
        endpoint = '/api/admin/therapists-pending';
      } else if (activeTab === 'approved') {
        endpoint = '/api/admin/therapists-approved';
      } else if (activeTab === 'rejected') {
        endpoint = '/api/admin/therapists-rejected';
      }
      
      console.log("Fetching from endpoint:", `http://localhost:5001${endpoint}`);
