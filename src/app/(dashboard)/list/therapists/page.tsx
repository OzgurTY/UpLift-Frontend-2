...
const TherapistListPage = () => {
    ...
    const renderRow = (item: Therapist) => (
        <tr key={item.id} className='border-b border-x-gray-200 even:bg-slate-50 text-sm hover:bg-upliftPurpleLight'>
            ...
            <td className='hidden md:table-cell'>{item.therapistId}</td>
            <td className='hidden md:table-cell'>{item.specialization.join(",")}</td>
            <td className='hidden md:table-cell'>{item.languages.join(",")}</td>
            ...
            <td>
                <div className='flex flex-row items-center gap-2'>
                    ...
                    {role === "admin" && (
                        <button className='w-7 h-7 flex items-center justify-center rounded-full bg-upliftPurple'>
    ...
    return (
        <div className='bg-white p-4 rounded-md flex-1 m-4 mt-0'>
            ...
            {/** LIST */}
            <Table columns={columns} renderRow={renderRow} data={therapistsData} />
            {/** PAGINATION */}
            <Pagination />
        </div>
    )
